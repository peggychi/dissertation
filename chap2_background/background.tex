...
A \keyword{tutorial} or a \keyword{how-to}, commonly called \keyword{instructions}, describes how to accomplish a specific \keyword{task} in various domains, including:
\begin{itemize}
  \itemsep -2pt
  \item Software application that, such as manipulating an image to create a specific blurring effect or creating a pie chart using a spreadsheet application,
...
%
Often, the content can be structured into a list of steps, which is usually in a linear order, presented as a \keyword{step-by-step} document.
%
Some domains have derived specific instructional formats, such as cooking recipes that contain not only actions but also food ingredients and their amounts.
%
Note that instructions are different from a (software) \keyword{documentation}, \keyword{manual}, or \keyword{user guide} that is a technical document contained non-task-centric details about a particular hardware or software system.

Instructions can be shown in different formats. Popular representations include a diagram, a static tutorial with text and images, or an instructional video. Section \ref{background_format} will introduce tutorial formats in details.
