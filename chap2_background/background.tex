...
\begin{figure*}[th!]
  \centering
  \includegraphics[width=\textwidth]{\background/fig/layering/IBMSeriesIIICopier}
...
Ordering of a series of actions can be expressed using numbers or spatial sequences, sometimes with arrows, in a clear layout.
%
Information should be visually concise and distinguishable. Take Figure~\ref{fig:background_marshalling} for example. Tufte pointed out the top design does not effectively separates the channels (the actor--in silhouette, the prop--the light with lamp-whiskers, the action--the directional arrows, the text, and the grid boxes of individual diagrams). The visuals are ``too loud and too similar.''
%
The revised design, shown at the bottom of the figure, separates the elements from the actor: the glowing light is colored in yellow and whiskers are removed (not to confused with trembling motion); motion arrows are highlighted in red; the weight of the grid border is lighted to avoid confusion with motion arrows; silhouette is shown in gray to bring more emphasis on other information; text is shown with a lighter font.
...
In addition to adding annotations to a shot, several conventional video editing techniques are often seen. A \textbf{close-up} is useful to zoom in a shot to demonstration a detailed view.
%
\textbf{Subtitles} are used to describe the content in text.
%
\textbf{Auditory effects} can guide learners .
%
When providing instructions, \emph{time} is an important factor to guide learners through .
But to provide concise information, time is often manipulated by jumping between shots, applying transitions, or being fast-forward or slowdown. Subtitles or text annotation to provide an indication of time
%
