...
Static, step-by-step instructions are composed of one or more figures, which can be either illustrations, images, or software screenshots. To help learners identify the key information, visual annotations are commonly applied to figures, often with text descriptions.
%
Table~\ref{background_techniques} shows a list of annotation techniques in practices that I summarized from book material~\cite{mijksenaar1999open,greenberg2012sketching,Buxton:2007:SUE:1526229,tufte1990envisioning}. The visual elements are used to indicate the movements of actions or objects (via arrows or paths), identify important parts (via highlights), present the details and overview (via call-outs), and provide additional messages such as a warning (via icons or text).

Ordering of a series of actions can be expressed using numbers, sometimes with arrows, in a clear layout.
%
Specifically, Tufte~\cite{tufte1990envisioning} pointed out the importance of ... \todo{the fig from p63 here}
...
\begin{table}[h!]
  \centering
  \includegraphics[width=0.8\columnwidth]{\background/fig/techniques/techniques}
  \caption{A list of annotation techniques that are often used to provide instructions.}
  \label{background_techniques}
\end{table}
...
To provide concise visual tutorials, \textbf{annotation} techniques of static instructions are commonly used, including applying arrows, paths, highlights, call-outs, and text~\cite{Chi:2013:DGC:2501988.2502052}. Vignette or dark edges are rare but could be used for highlighting a video subview (see Figure~\ref{fig:background_video_techniques}a).
%
In addition to adding annotations to a shot, a \textbf{close-up} is useful to zoom in a shot to demonstration a detailed view.
%
\textbf{Subtitles}
%
auditory effects
...
Time can be manipulated by jumping between shots, applying transitions, or being fast-forward or slowdown. Subtitles or text annotation to provide an indication of time
%
Therefore, needs editing expertise and time
